...
export default function TeamPage() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      await teamApi.invite(inviteEmail)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {loading ? (
          <div style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Loading members…</div>
        ) : members.length === 0 ? (
          <div style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>No members found</div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 20 }}>They will receive an invite token to join your AgentSec workspace.</div>
        {success && <div style={{ background: 'rgba(0,229,160,0.08)', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 7, padding: '9px 14px', color: 'var(--accent)', fontSize: 13, marginBottom: 14 }}>✓ {success}</div>}
        {error   && <div style={{ background: 'rgba(255,71,87,0.08)', border: '1px solid rgba(255,71,87,0.2)', borderRadius: 7, padding: '9px 14px', color: '#FF4757', fontSize: 13, marginBottom: 14 }}>{error}</div>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px', gap: 12 }}>
          {[
            ...
            <div key={i} style={{ display: 'contents' }}>
              {row.map((cell, j) => (
                <div key={j} style={{ fontSize: i === 0 ? 10 : 12, color: cell === '✓' ? 'var(--accent)' : cell === '✗' ? 'var(--text-muted)' : i === 0 && j === 0 ? 'var(--text-muted)' : j === 0 ? 'var(--text-sec)' : 'var(--text)', fontWeight: i === 0 ? 600 : 400, letterSpacing: i === 0 ? '0.4px' : 0, borderBottom: i < 5 ? '1px solid var(--border)' : 'none', paddingBottom: i < 5 ? 10 : 0, paddingTop: i > 0 ? 2 : 0, textAlign: j > 0 ? 'center' as const : 'left' as const }}>
                  {cell}
                </div>
              ))}
            </div>
