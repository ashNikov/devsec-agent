...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    await Promise.allSettled([
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/project/status`, { headers: { Authorization: `Bearer ${localStorage.getItem('agentsec_token')}` } }).then(r => r.json()).then(setProject),
